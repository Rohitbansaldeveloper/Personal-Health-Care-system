...
# Apply deployments using piping to preserve placeholders on disk
Write-Host "Applying backend deployment with dynamic image tag..."
(Get-Content k8s/backend-deployment.yaml) -replace '__BACKEND_IMAGE__', $BACKEND_IMAGE | kubectl apply -f -

Write-Host "Applying frontend deployment with dynamic image tag..."
(Get-Content k8s/frontend-deployment.yaml) -replace '__FRONTEND_IMAGE__', $FRONTEND_IMAGE | kubectl apply -f -

kubectl apply -f k8s/hpa.yaml

Write-Host "`nDeployment to Kubernetes initiated successfully!" -ForegroundColor Green
Write-Host "You can check the status of your pods by running: kubectl get pods -n healthcare-ns" -ForegroundColor Yellow
